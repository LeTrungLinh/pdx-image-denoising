    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for phase in ['train', 'val']:
            ...
            for step, (inputs, targets) in iterator:
                ...
                ################# show image to test inputs ################
                # # Remove first singleton dimension
                tensor = inputs.squeeze()
                ...
                for i in range(len(numpy_array)):
                    io.imsave(f"test_aug/test_{i}_{step}.jpg", numpy_array[i].transpose(1, 2, 0))
                # fig, ax = plt.subplots(nrows=4, ncols=4)
                # for i, axi in enumerate(ax.flat):
                #     if i <= 16:
                #         axi.imshow(numpy_array[i].transpose(1, 2, 0))
                #         axi.axis('off')
                # plt.show()
                # plt.imshow(numpy_array[1].transpose(1, 2, 0))
                # plt.show()
                #################################################################
